...
try:
    from app.core.config import FEATURE_COLUMNS, TRADE_SYMBOL
...
try:
    # Import only SessionLocal at runtime; Trade model may be used for typing only
    from app.db import SessionLocal

    # Attempt to import Trade for runtime use if available
    try:
...
st.set_page_config(
    page_title="OvieX Quant Engine Signal Trading Assistant",
    page_icon="📈",
    layout="wide",
...
# Title
st.markdown(
    "<h1 class='main-title'>📈 OvieX Quant Engine Signal Trading Assistant</h1>",
    unsafe_allow_html=True,
)
    ...
    if DEPENDENCIES["database"]:
        try:
            ...
            if db and TradeModel is not None:
                ...
                raise ImportError("Trade model is not available")
...
st.markdown(
    ...
    <div style='text-align: center; color: #666; font-size: 0.9rem;'>
        <p>📊 OvieX Quant Engine Trading Signal Assistant v0.0.1 |
